...
class Scaffolder:
    ...
    def __init__(self, destination: Path):
        ...
        try:
            ...
            self._error(
                "Could not find package data. Is the package installed correctly?"
            )
    ...
    def scaffold(self, force: bool = False) -> None:
        ...
        if self.destination_exists():
            if force:
                self._warn(
                    "Existing project files found. --force is enabled. Cleaning up first."
                )
                self.clean(prompt=False)
            else:
                self._error("Project files already exist in this directory. Aborting.")
                self._info(
                    "Use --force to overwrite or --clean to remove the existing structure."
                )
                return
        try:
    ...
    def clean(self, prompt: bool = True) -> None:
        ...
        if cleaned_count > 0:
            self._success(
                f"Cleanup complete. Removed approximately {cleaned_count} items."
            )
        else:
    ...
    def _set_executable_permissions(self) -> None:
        executable_scripts = [
            ...
            "scripts/02-scaffold-project.sh",
        ]
        for script_path_str in executable_scripts:
            ...
            if script_path.exists():
                try:
                    ...
                    self._warn(
                        f"Could not set executable permission on '{script_path}': {e}"
                    )

    def _info(self, message: str) -> None:
        print(f"{C_INFO}[INFO]{C_RESET} {message}")

    def _success(self, message: str) -> None:
        print(f"{C_SUCCESS}[SUCCESS]{C_RESET} {message}")

    def _warn(self, message: str) -> None:
        print(f"{C_WARN}[WARNING]{C_RESET} {message}")
...
def main() -> None:
    ...
    group.add_argument(
        ...
        "--force",
        action="store_true",
        help="Force overwrite. Deletes existing project structure before creating a new one.",
    )
    group.add_argument(
        "-c",
        "--clean",
        action="store_true",
        help="Clean up. Removes all files and directories associated with the project.",
    )
