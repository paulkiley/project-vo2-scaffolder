...
def cmd_list(_: argparse.Namespace) -> None:
    ...
    for wi in data.get("work_items", []):
        print(f"{wi['id']}\t{wi.get('priority','')}\t{wi.get('status','')}\t{wi['title']}")
...
def main() -> None:
    ...
    s1 = sub.add_parser("list"); s1.set_defaults(func=cmd_list)
    s2 = sub.add_parser("show"); s2.add_argument("id"); s2.set_defaults(func=cmd_show)
    s3 = sub.add_parser("validate"); s3.set_defaults(func=cmd_validate)
